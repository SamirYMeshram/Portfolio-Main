...
const prefillMessageButton = document.getElementById('prefillMessage');
...
const formFields = {
  ...
  project: document.getElementById('contactProject'),
  budget: document.getElementById('contactBudget'),
  tone: document.getElementById('contactTone'),
  timeline: document.getElementById('contactTimeline'),
  references: document.getElementById('contactReferences'),
...
if (prefillMessageButton && formFields.message) {
  prefillMessageButton.addEventListener('click', () => {
    const template = [
      'Project goal:',
      'I want a premium digital experience that feels more intentional, polished, and memorable than what I have now.',
      '',
      'Current situation:',
      'There is either an existing version that feels limited, or a new idea that needs structure and direction.',
      '',
      'Main priorities:',
      ...
      '',
      'Important notes:',
      'Please share recommendations on the best scope, timeline, and execution approach.'
    ].join('\n');
    formFields.message.value = template;
...
if (copyEmailButton) {
  copyEmailButton.addEventListener('click', async () => {
    try {
      await navigator.clipboard.writeText('samm9421@gmail.com');
...
if (contactForm) {
  contactForm.addEventListener('submit', (event) => {
    ...
    const name = formFields.name ? formFields.name.value.trim() : '';
    const email = formFields.email ? formFields.email.value.trim() : '';
    const project = formFields.project ? formFields.project.value.trim() : '';
    const budget = formFields.budget ? formFields.budget.value.trim() : '';
    const tone = formFields.tone ? formFields.tone.value.trim() : '';
    const timeline = formFields.timeline ? formFields.timeline.value.trim() : '';
    const references = formFields.references ? formFields.references.value.trim() : '';
    const message = formFields.message ? formFields.message.value.trim() : '';

    const subject = encodeURIComponent(`Project Inquiry • ${project || 'New project'} • ${name || 'Client'}`);
    const bodyText = [
      `Name: ${name}`,
      `Email: ${email}`,
      ...
      `References: ${references || 'None shared yet'}`,
      '',
      'Project Brief:',
      message
    ].join('\n');
    ...
    const mailto = `mailto:samm9421@gmail.com?subject=${subject}&body=${body}`;

    if (contactFormNote) {
      contactFormNote.textContent = 'Opening your email app with a structured draft now. Review it, tweak it, and send when ready.';
    }
